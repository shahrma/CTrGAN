...
def define_G(opt, input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal',
             gpu_ids=[], n_downsampling=2,pretrained_path=None):
    netG = None
    ...
    if which_model_netG == 'EdgeGen':
        ...
        n_res = 8
        mlp_dim = 128#SM256
        netG = CSGenerator(opt, input_nc, ngf, style_dim, n_downsampling, n_res, mlp_dim,pretrained_path=pretrained_path)
    else:
...
class BasicGenerator(nn.Module):
    ...
    def forward(self, input_EP1, input_BP2): #, input_BEP2
        """Standard forward"""
        x = torch.cat([input_EP1, input_BP2], dim=1) # , input_BEP2
        x = self.prepro(x)
        x = self.postpro(x)
        return x
