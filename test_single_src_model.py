...
def sm_test_run(opt_,outvis='fake_M0',source_id_model=None) :
    ...
    if source_id_model is None :
        results_dir = os.path.join(opt.results_dir, name, 'results',f'ep_{int(opt.which_epoch):05}','rgb', f'T{opt.target_id}',opt.source_id)
    else :
        results_dir = os.path.join(opt.results_dir, name,f'MODEL_T{opt.target_id}_S{source_id_model}', 'results', f'ep_{int(opt.which_epoch):05}', 'rgb', f'T{opt.target_id}', opt.source_id)
...
if __name__ == '__main__':
    ...
    for target_id in TARGET_LIST:
        for source_id in SOURCE_LIST :
            ...
            #opt.name = os.path.join(name , f'T{opt.target_id}')
            sm_test_run(opt,outvis='fake_B0',source_id_model= source_id_model)
